...
float hybrid_score(const float* p,
                   ...
                   float        alpha)
{
    /* ---------- BLOSUM part (dot product) ------------------------- */
    __m256 acc = _mm256_setzero_ps();
    for (int k = 0; k < 16; k += 8) {
        __m256 pv = _mm256_loadu_ps(p + k);
        __m256 qv = _mm256_loadu_ps(q + k);
        __m256 mv = _mm256_loadu_ps(M + k * 20);      // 20×20 row-major
    ...
    float blos_dist = 1.f - blosum;                   /* ∈ [0,1] */
    ...
    if (!std::isfinite(dist)) dist = 1e6f;            // large penalty
    return dist;
}
...
/* ================================================================== */
/*  Needleman–Wunsch affine alignment                                  */
/* ================================================================== */
AlignmentResult
          ...
          float alpha)
{
    ...
    /* ---------- rolling DP rows (current / previous) ------------- */
    std::vector<f32> Mp(L2 + 1, NEG_INF), Xp(L2 + 1, NEG_INF),
                     Yp(L2 + 1, NEG_INF),
    ...
    /* ---------- full score matrices for traceback gap tests ------ */
    std::vector<f32> MM((L1 + 1) * (L2 + 1), NEG_INF);
    std::vector<f32> XX((L1 + 1) * (L2 + 1), NEG_INF);
    std::vector<f32> YY((L1 + 1) * (L2 + 1), NEG_INF);
    ...
    bufA.reserve((L1 + L2) * 21);
    bufB.reserve((L1 + L2) * 21);

    int      i = L1, j = L2;
    uint8_t  state = TB[idx(i, j)];            // first move
